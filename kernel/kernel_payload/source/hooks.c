...
void md5(uint8_t *initial_msg, uint64_t initial_len, char* md5_final_hash) {
    ...
    msg = malloc(new_len + 64, M_TEMP, 2); // also appends "0" bits (we alloc also 64 extra bytes...)
...
int sys_ioctl_hook(struct thread *td, struct ioctl_args *uap) {
    switch (uap->com) {
        case 0xFFFFFFFFC0184401:
        case 0xC0184401 : { // Decrypt PUP Header
            // Copy argument
            ...
            char md5_data[33];
            md5(header_args.buffer, header_args.length, md5_data);

            printf("IOCTL(0xC0184401): Decrypt PUP Header called.\n");
            printf("MD5: %s\n", md5_data);

            char file[255];
            snprintf(file, 255, "/mnt/usb1/decrypt/%s.dec", md5_data);
        ...
        case 0xFFFFFFFFC0184402:
        case 0xC0184402 : { // Verify segment (1)
        ...
        case 0xFFFFFFFFC0184403:
        case 0xC0184403 : { // Verify segment (2)
        ...
        case 0xFFFFFFFFC010440D:
        case 0xC010440D : { // Verify BLS Header (3)
        ...
        case 0xFFFFFFFFC0184404:
        case 0xC0184404 : { // Decrypt segment
            // Copy argument
            ...
            // Create the MD5 digest of the PUP Header
            char md5_data[33];
            md5(decrypt_args.buffer, decrypt_args.length, md5_data);
        ...
        case 0xFFFFFFFFC0284405:
        case 0xC0284405 : { // Decrypt segment block
            // Copy argument
            ...
            // Create the MD5 digest of the PUP Header
            char md5_data[33];
            md5(block_buffer, decrypt_block_args.block_length, md5_data);
    ...
    return sys_ioctl_orig(td, uap);
}


// Jailbreak syscall
int sys_jailbreak(struct thread *td, void* uap) {
    ...
    *sceProcCap = 0xFFFFFFFFFFFFFFFFULL; // Sce Process
    ...
    printf("Process succefully jailbreak !\n");

    td->td_retval[0] = 0;
    return 0;
}

// Installing everything in kernel
int install_hooks() {
    // Resolve syscall function
    sys_ioctl_orig = get_syscall_function(54);

    initSysWrapper();

    cpu_disable_wp();

    // 1. Install the Ioctl hook
    install_syscall(54, sys_ioctl_hook);

    // 2. Install the jailbreak
    install_syscall(68, sys_jailbreak);
